...
from edr_pydantic.capabilities import LandingPageModel, Link, Contact
...
def setup_logging():
    ...
    formatter = logging.Formatter(
        "%(asctime)s ; edr-api ; %(process)s ; %(levelname)s ; %(name)s ; %(message)s"
    )
...
async def landing_page(request: Request) -> LandingPageModel:
    return LandingPageModel(
        title="EDR tutorial",
        description="Tutorial for the OGC API - Environmental Data Retrieval Standard",
        links=[
            Link(
            ...
            Link(
                ...
                rel="service-desc",
                title="Documentation of API in http",
            ),
            Link(
                href="http://localhost:8000/openapi.json",
                rel="service-desc",
                title="Documentation in JSON",
            ),
        ],
        keywords="API EDR".split(),
        provider=None,
        contact=Contact(email="joran.angevaare@knmi.nl"),
    )
